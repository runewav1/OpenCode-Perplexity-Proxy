...
## Authentication
The server.ts uses an environment variable "PERPLEXITY_API_KEY". You'll need to create a .env file inside the "pplx-proxy" directory, and inside of it, you need to define the variable as so: "PERPLEXITY_API_KEY=YOUR_API_KEY" (Obviously, replace "YOUR_API_KEY" with an actual API key).
